...
public class ChessController {
	...
	class SearchListener implements Search.Listener {
		...
		public void notifyPV(int depth, int score, int time, int nodes,
	...
	public String getPGN() {
		...
		pgn.append(String.format("[Date \"%04d.%02d.%02d\"]%n", year, month,
				day));
		String white = "Player";
	...
	final public boolean doMove(Move move) {
		...
		for (int mi = 0; mi < moves.size; mi++) {
			...
			if ((m.from == move.from) && (m.to == move.to)) {
				// promotion expected, sneakily make white queen the requested
				// promotion
				if (humansTurn() && m.promoteTo != Piece.EMPTY) {
	...
	private void startComputerThinking() {
		if (game.pos.whiteMove != humanIsWhite) {
			...
			thinkingPV = "";
			setStatusString();
			// updateGUI(); // don't implicitly update image here; we'll do it
			// in next step
			setSelection();
		}
	}

	private void startComputerThinkingAsync() {
		if (game.pos.whiteMove != humanIsWhite) {
			if (computerThread == null) {
